...
class IonAuthModel{
    ...
    public function setSession(\stdClass $user): bool {
        ...
        $sessionData = [
            ...
            'last_check'          => time(),
            'foto'       		  => ($user->img != '') ? base_url('/assets/img/'.$user->img) : base_url('/assets/img/avatar_l.png')
        ];
    ...
    public function get_datatables(){
        ...
        return $this->response->get()->getResult();
    }
    public function count_filtered(){
        $this->triggerEvents('count_filtered');
        $this->_get_datatables();
        return $this->response->get()->getNumRows();
    }
    public function count_all(){
        $this->triggerEvents('count_all');
        $this->_get_datatables();
        return $this->response->countAllResults();
    }
    //Group
    ...
    public function deactivate(int $id=0): bool{
		...
		if (! $id){
			...
			return false;
		}else if ($this->recheckSession() && $user_id == $id){
    ...
    public function upload_pdf($file,$file_name, $file_old, int $max_size = null) : bool{
        $type = $file->getClientMimeType();
        $size = $file->getSize('mb');
        ...
        $resize_h = isset($resize_h) ? $resize_h : '250';
        $resize_w = isset($resize_w) ? $resize_w : '200';
        if ($type != 'application/pdf') {	// File Tipe Sesuai
            $this->setError('Extensi File Tidak Diizinkan.. Gunakan file .pdf (Pastikan extensinya menggunakan huruf kecil)');
            return false;
        }
        if($size <= $max_size) {
            $this->setError('Ukuran File Yang Anda Upload Melebihi Batas Maximum');
        ...
        helper('filesystem'); // Load Helper File System
        $direktori = ROOTPATH . 'public/pdf/'; //definisikan direktori upload
        if(!empty($file_old)){
            delete_files($direktori , $file_old); //Hapus terlebih dahulu jika file ada
        }
        $upload = $file->move($direktori,$file_name);
        if ($upload) {
            $this->setMessage('file berhasil diuploads');
            return true;
        }
